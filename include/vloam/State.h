...
{
private:
    Pose3 pose_;       // Rotation from imu to global, Position of imu in global
...
public:
    // Default Constructor
    State()
        : pose_(Pose3()), velocity_(Vector3()), bias_(Bias())
    {}
    ...
    // Return gyroscope bias as Vectot3
    ...
    {
        os << "[STATE]: q=" << std::fixed
           << state.q().x() << ", " << std::fixed << state.q().y() << ", "
           << state.q().z() << ", " << std::fixed << state.q().w() << "| ";
